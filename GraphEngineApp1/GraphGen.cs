...
namespace GraphGenerator
{
    ...
    class GraphGen
    {
        ...
        void CreateEdgeThreadProc(object par)
        {
            ...
            long start;
            long end;
            if (p.threadCount - 1 == p.threadIndex)//最后一个thread
            {
                start = 0;
        ...
        public void CreatGraph()
        {
            ...
            for (int threadIndex = 0; threadIndex < threadCount; threadIndex++)
            {
                ...
                threadNum[threadIndex].Start(p);//多线程
            }
            ...
            for (int inde = 0; inde < threadCount; inde++)
                threadNum[inde].Join();
            watch.Stop();
            Console.WriteLine("Remove dirty outlinks time:{0}",watch.ElapsedMilliseconds);
